...
>(({ className, children, ...props }, ref) => (
  ...
  >
    <span className='absolute left-2 flex h-3.5 w-3.5 items-center justify-center'>
      <DropdownMenuPrimitive.ItemIndicator>
        <svg className='h-2 w-2 fill-current' viewBox='0 0 8 8' xmlns='http://www.w3.org/2000/svg'>
...
>(({ className, size, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn(dropdownMenuLabelVariants({ size }), inset && 'pl-8', className)}
    {...props}
  />
));
...
const DropdownMenuShortcut = ({
  ...
  variant,
  ...props
}: React.HTMLAttributes<HTMLSpanElement> & VariantProps<typeof dropdownMenuShortcutVariants>) => {
  return <span className={cn(dropdownMenuShortcutVariants({ variant }), className)} {...props} />;
};
